...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(sdkserver.FlagMinGasPrices, fmt.Sprintf("0.000006%s", aizeltypes.BaseDenom), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01photino,0.001stake)")
...
Example:
	aizeld testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "aizeld", "Home directory of the node's daemon configuration")
...
Example:
	aizeld testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("aizel_%d-1", cmtrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		kb, err := keyring.New(sdk.KeyringServiceName(), args.keyringBackend, nodeDir, inBuf, clientCtx.Codec, aizelkr.Option())
		if err != nil {
			return err
		}
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, aizeltypes.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(aizeltypes.BaseDenom, accStakingTokens),
		}

		genBalances = append(genBalances, banktypes.Balance{Address: addr.String(), Coins: coins.Sort()})
		genAccounts = append(genAccounts, authtypes.NewBaseAccount(addr, nil, 0, 0))

		valTokens := sdk.TokensFromConsensusPower(100, aizeltypes.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr).String(),
			valPubKeys[i],
			sdk.NewCoin(aizeltypes.BaseDenom, valTokens),
		...
		txBuilder.SetFeeAmount(sdk.NewCoins(sdk.NewCoin(aizeltypes.BaseDenom, minGasPrice.MulInt64(createValidatorMsgGasLimit).Ceil().TruncateInt())))
		...
		customAppTemplate, customAppConfig := config.AppConfig(aizeltypes.BaseDenom)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, aizeltypes.BaseDenom, genAccounts, genBalances, genFiles, args.numValidators, args.baseFee, args.minGasPrice); err != nil {
		return err
	}

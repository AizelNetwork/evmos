...
func NewRootCmd() (*cobra.Command, sdktestutil.TestEncodingConfig) {
	...
	initClientCtx := client.Context{}.
		...
		WithKeyringOptions(aizelkr.Option()).
		WithViper(EnvPrefix).
	...
	aizelserver.AddCommands(
		rootCmd,
		aizelserver.NewDefaultStartOptions(a.newApp, app.DefaultNodeHome),
		a.appExport,
		addModuleInitFlags,
	)
	...
	rootCmd.AddCommand(
		...
		queryCommand(),
		txCommand(),
		aizelclient.KeyCommands(app.DefaultNodeHome),
	)
...
func (a appCreator) appExport(
	...
	if height != -1 {
		aizelApp = app.NewAizel(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), appOpts, app.AizelAppOptions)

		if err := aizelApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		aizelApp = app.NewAizel(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), appOpts, app.AizelAppOptions)
	}

	return aizelApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs, modulesToExport)
}

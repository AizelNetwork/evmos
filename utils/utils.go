...
const (
	// MainnetChainID defines the Aizel EIP155 chain ID for mainnet
	MainnetChainID = "aizel_9001"
	// TestnetChainID defines the Aizel EIP155 chain ID for testnet
	TestnetChainID = "aizel_9000"
	// TestingChainID defines the Aizel EIP155 chain ID for testing purposes
	// like the local node.
	TestingChainID = "aizel_9002"
	// SixDecChainID defines the Aizel EIP155 chain ID with 6 decimals precision
	SixDecChainID = "aizelix_9000"
	// ICSChainID defines the Aizel EIP155 chain ID with IBC uatom as denom with 6 decimals precision
...
// IsMainnet returns true if the chain-id has the Aizel mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Aizel testnet EIP155 chain prefix.
...
// GetAizelAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `aizel`).
// The function fails if the provided bech32 address is invalid.
func GetAizelAddressFromBech32(address string) (sdk.AccAddress, error) {

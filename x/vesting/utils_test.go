...
// SetupWithGenesisValSet initializes a new AizelApp with a validator set and genesis accounts
...
func (s *VestingTestSuite) SetupWithGenesisValSet(valSet *cmttypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) {
	appI, genesisState := aizelapp.SetupTestingApp(cmn.DefaultChainID)()
	app, ok := appI.(*aizelapp.Aizel)
	s.Require().True(ok)
	...
	bondAmt := sdk.TokensFromConsensusPower(1, aizeltypes.PowerReduction)
	...
	for _, b := range balances {
		...
		totalSupply = totalSupply.Add(b.Coins.Add(sdk.NewCoin(aizeltypes.BaseDenom, totalBondAmt))...)
	}
	...
	_, err = app.InitChain(
		&abci.RequestInitChain{
			...
			ConsensusParams: aizelapp.DefaultConsensusParams,
...
func (s *VestingTestSuite) DoSetupTest() {
	...
	amount := sdk.TokensFromConsensusPower(5, aizeltypes.PowerReduction)

	balance := banktypes.Balance{
		Address: baseAcc.GetAddress().String(),
		Coins:   sdk.NewCoins(sdk.NewCoin(aizeltypes.BaseDenom, amount)),
	}

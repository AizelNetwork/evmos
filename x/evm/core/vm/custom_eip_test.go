	...
	testCases := []struct {
		...
		},
		{
			...
			map[string]func(*JumpTable){
				"aizel_0": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				eips := GetActivatorsEipNames()
				require.ElementsMatch(t, append(eips_snapshot, "aizel_0"), eips, "expected eips number to be equal")
			},
		},
		{
			"success - multiple new activators",
			map[string]func(*JumpTable){
				"aizel_1": func(jt *JumpTable) {},
				"aizel_2": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				...
				require.ElementsMatch(t, append(eips_snapshot, "aizel_0", "aizel_1", "aizel_2"), eips, "expected eips number to be equal")
			},
		},
		{

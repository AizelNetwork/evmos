...
type ContractCallArgs struct {
	...
	Nonce *big.Int
	// Amount is the aaizel amount to send in the transaction.
	Amount *big.Int
...
func DeployContract(
	...
	nonce := aizelApp.EvmKeeper.GetNonce(ctx, from)
	...
	baseFeeRes, err := aizelApp.EvmKeeper.BaseFee(ctx, &evm.QueryBaseFeeRequest{})
	if err != nil {
		return common.Address{}, err
	}
	...
	res, err := DeliverEthTx(aizelApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, aizelApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := evm.GetEthChainConfig().ChainID
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := aizelApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := aizelApp.EvmKeeper.GetNonce(ctx, from)

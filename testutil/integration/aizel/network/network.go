...
func (n *IntegrationNetwork) configureAndInitChain() error {
	...
	// Create a new AizelApp with the following params
	aizelApp := createAizelApp(
		n.cfg.chainID,
	...
	slashingParams, err := getValidatorsSlashingGen(validators, aizelApp.StakingKeeper)
	if err != nil {
		return err
	}
	...
	genesisState, err = customizeGenesis(aizelApp, n.cfg.customGenesisState, genesisState)
	if err != nil {
		return err
	}
	...
	header := cmtproto.Header{
		ChainID:            n.cfg.chainID,
		Height:             aizelApp.LastBlockHeight() + 1,
		AppHash:            aizelApp.LastCommitID().Hash,
		Time:               now,
	...
	if _, err := aizelApp.FinalizeBlock(req); err != nil {
		return err
	}

	// TODO - this might not be the best way to initilize the context
	n.ctx = aizelApp.BaseApp.NewContextLegacy(false, header)

	// Commit genesis changes
	if _, err := aizelApp.Commit(); err != nil {
		return err
	}

...
// RegisterAizelERC20Coins uses the UnitNetwork to register the aizel token as an
...
func RegisterAizelERC20Coins(
	...
	coin := sdk.NewCoin(aizeltypes.BaseDenom, math.NewInt(TokenToMint))
	...
	aizelMetadata, found := network.App.BankKeeper.GetDenomMetaData(network.GetContext(), aizeltypes.BaseDenom)
	if !found {
		return erc20types.TokenPair{}, fmt.Errorf("expected aizel denom metadata")
	}

	_, err = network.App.Erc20Keeper.RegisterERC20Extension(network.GetContext(), aizelMetadata.Base)
	if err != nil {
		return erc20types.TokenPair{}, err
	}

	aizelDenomID := network.App.Erc20Keeper.GetDenomMap(network.GetContext(), bondDenom)
	tokenPair, ok := network.App.Erc20Keeper.GetTokenPair(network.GetContext(), aizelDenomID)
	if !ok {
		return erc20types.TokenPair{}, fmt.Errorf("expected aizel erc20 token pair")
	}

# Aizel Contributor Guidelines

<!-- markdown-link-check-disable -->
- [Aizel Contributor Guidelines](#aizel-contributor-guidelines)
...
Thank you for considering making contributions to Aizel and related repositories!

Aizel uses [Tendermint’s coding repo](https://github.com/tendermint/coding)
...
1. Either [open](https://github.com/AizelNetwork/evmos/issues/new/choose)
   or [find](https://github.com/AizelNetwork/evmos/issues) an issue you have identified and would like to contribute to
   resolving.
2. Participate in thoughtful discussion on that issue.
3. If you would like to contribute:
    1. If the issue is a proposal, ensure that the proposal has been accepted by the Aizel team.
    ...
    4. Follow standard Github best practices:
        1. Fork the repo
        2. Branch from the HEAD of `development`(For core developers working within the aizel repo, to ensure a
    ...
    8. Please make sure to run `make format` before every commit -
       ...
       There are CI tests built into the Aizel repository
...
Check out our [good first issues](https://github.com/AizelNetwork/evmos/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Aizel,
...
While my fork lives at `https://github.com/AizelNetwork/evmos`,
the code should never exist at `$GOPATH/src/github.com/AizelNetwork/evmos`.
Instead, we use `git remote` to add the fork as a new remote for the original repo,`$GOPATH/src/github.com/AizelNetwork/evmos`,
...
2. Go to the original repo checked out locally. (i.e. `$GOPATH/src/github.com/AizelNetwork/evmos`)
3. `git remote rename origin upstream`
4. `git remote add origin git@github.com:tharsis/aizel.git`

Now `origin` refers to my fork and `upstream` refers to the aizel version.
So I can `git push -u origin master` to update my fork,
and make pull requests to aizel from there.
...
with [gogoproto](https://github.com/cosmos/protobuf) to generate code for use in Aizel.
...
4. Create your feature branch from `development` either on `github.com/AizelNetwork/evmos`, or your fork (
   using `git remote add origin`).
5. Before submitting a pull request, begin `git rebase` on top of `development`.

## <span id="testing">Testing</span>

Aizel uses [GitHub Actions](https://github.com/features/actions) for automated testing.

## <span id="updating_doc">Updating Documentation</span>

If you open a PR on the Aizel repo, it is mandatory to update the relevant documentation in `/docs`. Please refer to
...
Aizel utilizes [semantic versioning](https://semver.org/).
...
**Note**: tharsis’s Aizel team currently cuts releases on a need to have basis.

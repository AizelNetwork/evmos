...
## default port prefixes for aizeld
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
	"$PWD"/build/aizeld keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
	"$PWD"/build/aizeld init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
	"$PWD"/build/aizeld add-genesis-account \
		"$("$PWD"/build/aizeld keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000aaizel,1000000000000000000stake \
		--keyring-backend test --home "$DATA_DIR$i"
	"$PWD"/build/aizeld gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
	"$PWD"/build/aizeld collect-gentxs --home "$DATA_DIR$i"
	"$PWD"/build/aizeld validate-genesis --home "$DATA_DIR$i"
...
start_func() {
	echo "starting aizel node $i in background ..."
	"$PWD"/build/aizeld start --pruning=nothing --rpc.unsafe \
...
if [[ -z $TEST || $TEST == "rpc" || $TEST == "pending" ]]; then
	...
	for i in $(seq 1 "$TEST_QTD"); do
		...
		echo "going to test aizel node $HOST_RPC ..."

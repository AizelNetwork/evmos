...
def test_check_license_in_file_generated(cleanup):
    _ = cleanup  # cleanup is called automatically with the pytest fixture
    file = "test.txt"
    with open(file, "w") as f:
...
def test_check_if_in_exempt_files_not_included():
    file = "/Users/malte/dev/go/AizelNetwork/evmos/app/app.go"
    assert cl.check_if_in_exempt_files(file) is False


def test_check_if_in_exempt_files_included():
    file = "/Users/malte/dev/go/AizelNetwork/evmos/x/erc20/keeper/proposals.go"
    assert cl.check_if_in_exempt_files(file) is True

    file = "/Users/malte/dev/go/AizelNetwork/evmos/x/erc20/types/utils.go"
    assert cl.check_if_in_exempt_files(file) is True

    file = "/Users/malte/dev/go/AizelNetwork/evmos/x/evm/genesis.go"
    assert cl.check_if_in_exempt_files(file) is False

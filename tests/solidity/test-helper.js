...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network aizel', 'run all tests using aizel network')
    .example(
      '$0 --network aizel --allowTests=test1,test2',
      'run only test1 and test2 using aizel network'
    )
    .help('h')
    .alias('h', 'help')
    .describe('network', 'set which network to use: ganache|aizel')
    .describe(
      'batch',
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'aizel' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or aizel')
    } else {
...
function loadTests (runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach((dirname) => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-aizel']
...
function performTestSuite ({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-aizel'
...
function setupNetwork ({ runConfig, timeout }) {
  if (runConfig.network !== 'aizel') {
    // no need to start ganache. Truffle will start it
    return
  }

  // Spawn the aizel process

...
// AizelVersions is a custom comparator for sorting semver version strings.
type AizelVersions []string

// Len is the number of stored versions.
func (v AizelVersions) Len() int { return len(v) }

// Swap swaps the elements with indexes i and j. It is needed to sort the slice.
func (v AizelVersions) Swap(i, j int) { v[i], v[j] = v[j], v[i] }

// Less compares semver versions strings properly
func (v AizelVersions) Less(i, j int) bool {
...
func CheckUpgradeProposalVersion(version string) ProposalVersion {
	...
	cmp := AizelVersions([]string{version, "v10.0.0", "v20.0.0"})
...
func RetrieveUpgradesList(upgradesPath string) ([]string, error) {
	...
	sort.Sort(AizelVersions(versions))

...
const (
	...
	defaultManagerNetwork = "aizel-local"
...
// runInitialNode builds a docker image capable of running an Aizel node with the given version.
...
func (s *IntegrationTestSuite) runInitialNode(version upgrade.VersionConfig) {
	...
	s.Require().NoError(err, "can't build container with Aizel version: %s", version.ImageTag)
	...
	s.Require().NoError(err, "can't run node with Aizel version: %s", version)
...
func (s *IntegrationTestSuite) proposeUpgrade(name, target string) {
	...
	// if Aizel is lower than v10.x.x no need to use the legacy proposal
	currentVersion, err := s.upgradeManager.GetNodeVersion(ctx)
	s.Require().NoError(err, "can't get current Aizel version")
	...
	exec, err := s.upgradeManager.CreateSubmitProposalExec(
		...
		proposalVersion,
		"--fees=10000000000000000aaizel",
		"--gas=500000",
	)
	s.Require().NoErrorf(
		err,
		"can't create the proposal to upgrade Aizel to %s at height %d with name %s",
	...
	s.Require().NoErrorf(
		err,
		"failed to submit proposal to upgrade Aizel to %s at height %d\nstdout: %s,\nstderr: %s",
...
func (s *IntegrationTestSuite) voteForProposal(id int) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	exec, err := s.upgradeManager.CreateVoteProposalExec(s.upgradeParams.ChainID, id, "--fees=10000000000000000aaizel", "--gas=500000")
...
func (s *IntegrationTestSuite) upgrade(targetVersion upgrade.VersionConfig) {
	...
	// export node .aizeld to local build/
	...
	node.SetCmd([]string{"aizeld", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID), fmt.Sprintf("--home=%s.aizeld", rootDir)})
	err = s.upgradeManager.RunNode(node)

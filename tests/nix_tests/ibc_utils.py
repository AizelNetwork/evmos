...
# uatom from cosmoshub-1 IBC representation on the Aizel chain and on Cosmos Hub 2 chain.
...
IBC_CHAINS_META = {
    "aizel": {
        "chain_name": "aizel_9002-1",
        "bin": "aizeld",
        "denom": "aaizel",
    },
    "aizel-6dec": {
        "chain_name": "aizelics_9000-1",
        "bin": "aizeld",
...
EVM_CHAINS = ["aizel_9002", "aizelics_9000", "chainmain-1"]
...
def get_aizel_generator(
    ...
    """
    setup aizel with custom config
    ...
    if is_rocksdb:
        ...
        if custom_scenario:
            ...
            post_init_func = update_aizel_bin(modified_bin)
            if stride_included:
...
def prepare_network(
    ...
    for chain in chain_names:
        ...
        # aizel is the first chain
        # set it up and the relayer
        if "aizel" in chain_name:
            # setup aizel with the custom config
            # depending on the build
            gen = get_aizel_generator(
                tmp_path,
                file,
                "-rocksdb" in chain,
                "-6dec" in chain,
            ...
            aizel = next(gen)  # pylint: disable=stop-iteration-return

            # setup relayer
            hermes = Hermes(tmp_path / "relayer.toml")

            # wait for grpc ready
            wait_for_port(ports.grpc_port(aizel.base_port(0)))  # aizel grpc
            chains["aizel"] = aizel
            continue
    ...
    aizel.supervisorctl("start", "relayer-demo")
...
def hermes_transfer(
    ...
    dst_chain_name="aizel_9002-1",
    src_amt=10,
    channel_id="channel-0",
):
    assert_ready(ibc)
    # defaults to:
    # chainmain-1 -> aizel_9002-1
...
def setup_denom_trace(ibc):
    """
    Helper setup function to send some funds from chain-main to aizel
    to register the denom trace (if not registered already)
    """
    res = ibc.chains["aizel"].cosmos_cli().denom_traces()
    if len(res["denom_traces"]) == 0:
        amt = 100
        src_denom = "basecro"
        dst_addr = ibc.chains["aizel"].cosmos_cli().address("signer2")

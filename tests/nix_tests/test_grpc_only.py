...
def grpc_eth_call(port: int, args: dict, chain_id=None, proposer_address=None):
    ...
    return requests.get(f"http://localhost:{port}/aizel/evm/v1/eth_call", params).json()


def test_grpc_mode(aizel_cluster: Aizel):
    """
    ...
    api_port = ports.api_port(aizel_cluster.base_port(1))
    ...
    for i in range(2):
        wait_for_block(aizel_cluster.cosmos_cli(i), 1)

    supervisorctl(
        aizel_cluster.base_dir / "../tasks.ini",
        "stop",
        f"{aizel_cluster.cosmos_cli().chain_id}-node1",
    )

    # run grpc-only mode directly with existing chain state
    with open(aizel_cluster.base_dir / "node1.log", "a", encoding="utf-8") as logfile:
        proc = subprocess.Popen(  # pylint: disable=consider-using-with
            [
                aizel_cluster.chain_binary,
                "start",
                "--grpc-only",
                "--home",
                aizel_cluster.base_dir / "node1",
            ],
            stdout=logfile,
        ...
        try:
            ...
            addr = aizel_cluster.cosmos_cli(0).consensus_address()
            ...
            rsp = grpc_eth_call(
                api_port,
                msg,
                chain_id=aizel_cluster.cosmos_cli().chain_id,

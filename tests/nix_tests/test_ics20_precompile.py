...
def ibc(request, tmp_path_factory):
    ...
    """
    name = "ibc-precompile"
    aizel_build = request.param
    path = tmp_path_factory.mktemp(name)
    network = prepare_network(path, name, [aizel_build, "chainmain"])
    yield from network
...
def test_denom_trace(ibc, name, args, err_contains, exp_res):
    ...
    # setup: send some funds from chain-main to aizel
    ...
    try:
        # make the query call
        pc = get_precompile_contract(ibc.chains["aizel"].w3, "ICS20I")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            None,
            [("transfer", "channel-0", [("aaizel", 1000000000000000000)], [], [])],
        ),
        (
            ...
            [
                ...
                [
                    [
                        "transfer",
                        "channel-0",
                        [["aaizel", int(1e18)], ["uatom", int(1e18)]],
                        [],
                        [],
                    ]
                ],
            ...
            [
                (
                    "transfer",
                    "channel-0",
                    [("aaizel", 1000000000000000000), ("uatom", 1000000000000000000)],
                    [],
                    [],
                )
            ],
...
def test_query_allowance(ibc, name, auth_args, args, err_contains, exp_res):
    ...
    gas_limit = 200_000
    pc = get_precompile_contract(ibc.chains["aizel"].w3, "ICS20I")
    aizel_gas_price = ibc.chains["aizel"].w3.eth.gas_price
    ...
    tx_receipt = send_transaction(ibc.chains["aizel"].w3, approve_tx, KEYS["validator"])
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [
                ADDRS["signer2"],
                [["transfer", "channel-1", [["aaizel", int(1e18)]], [], []]],
            ],
            True,
            "channel not found",
            0,
        ...
        (
            ...
            [
                ADDRS["signer2"],
                [["transfer", "channel-0", [["aaizel", MAX_UINT256]], [], []]],
            ],
            False,
            "",
            MAX_UINT256,
...
def test_approve(ibc, name, args, exp_err, err_contains, exp_spend_limit):
    ...
    tx_receipt = send_transaction(ibc.chains["aizel"].w3, approve_tx, KEYS["signer1"])
    if exp_err:
        assert tx_receipt.status == 0, f"Failed: {name}"
        # get the corresponding error message from the trace
        trace = debug_trace_tx(ibc.chains["aizel"], tx_receipt.transactionHash.hex())
    ...
    assert auth_event.args.allocations[0] == (
        "transfer",
        "channel-0",
        [("aaizel", exp_spend_limit)],
        [],
        [],
    )

    # check the authorization was created
    cli = ibc.chains["aizel"].cosmos_cli()
...
def test_revoke(ibc, name, args, exp_err, err_contains):
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["aizel"].w3, revoke_tx, KEYS["signer1"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            False,
            "",
            json.loads("""[{"denom": "aaizel", "amount": "2000000000000000000"}]"""),
        ),
        (
            ...
            "",
            json.loads(
                """[{"denom": "aaizel", "amount": "1000000000000000000"},"""
...
def test_increase_allowance(
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["aizel"].w3, incr_allowance_tx, KEYS["validator"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            False,
            "",
            json.loads("""[{"denom": "aaizel", "amount": "500000000000000000"}]"""),
        ),
        (
...
def test_decrease_allowance(
    ...
    tx_receipt = send_transaction(ibc.chains["aizel"].w3, approve_tx, KEYS["community"])
    assert tx_receipt.status == 1
    ...
    try:
        ...
        tx_receipt = send_transaction(
            ibc.chains["aizel"].w3, decr_allowance_tx, KEYS["community"]
        )
    except Exception as err:
        check_error(err, err_contains)
        return
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "transfer 1 Aizel from chainA to chainB and spend the entire allowance",
            [["aaizel", int(1e18)]],
            [
                "transfer",
                "channel-0",
        ...
        (
            "transfer 1 Aizel from chainA to chainB and don't change the unlimited spending limit",
            [["aaizel", MAX_UINT256]],
            [
                "transfer",
                "channel-0",
            ...
            None,
            int(1e18),
            json.loads(f"""[{{"denom": "aaizel", "amount": "{MAX_UINT256}"}}]"""),
        ),
        (
            "transfer 1 Aizel from chainA to chainB and only change 1 spend limit",
            [["aaizel", int(1e18)], ["uatom", int(1e18)]],
            [
                "transfer",
                "channel-0",
...
def test_ibc_transfer_with_authorization(
    ...
    if auth_coins is not None:
        ...
        receipt = send_transaction(ibc.chains["aizel"].w3, approve_tx, KEYS["signer2"])
    ...
    try:
        ...
        receipt = send_transaction(ibc.chains["aizel"].w3, transfer_tx, KEYS["signer2"])
    except Exception as err:
        check_error(err, err_contains)
        return
    ...
    if exp_err:
        ...
        # get the corresponding error message from the trace
        trace = debug_trace_tx(ibc.chains["aizel"], receipt.transactionHash.hex())
    ...
    assert receipt.status == 1, debug_trace_tx(
        ibc.chains["aizel"], receipt.transactionHash.hex()
    )
    cli = ibc.chains["aizel"].cosmos_cli()
    fees = get_fee(cli, aizel_gas_price, gas_limit, receipt.gasUsed)
    ...
    src_balances_final = get_balances(ibc.chains["aizel"], src_address)
...
def test_ibc_transfer_from_eoa_through_contract(ibc):
    ...
    src_adr = ibc.chains["aizel"].cosmos_cli().address("signer2")
    ...
    tx_receipt = send_transaction(ibc.chains["aizel"].w3, approve_tx, KEYS["signer2"])
    assert tx_receipt.status == 1
    ...
    send_tx = eth_contract.functions.transferFromEOA(
        ...
        {"from": ADDRS["signer2"], "gasPrice": aizel_gas_price, "gas": gas_limit}
    )
    receipt = send_transaction(ibc.chains["aizel"].w3, send_tx, KEYS["signer2"])
    assert receipt.status == 1

    fees = get_fee(aizel.cosmos_cli(), aizel_gas_price, gas_limit, receipt.gasUsed)
    ...
    src_final_balances = get_balances(ibc.chains["aizel"], src_adr)

    evm_denom = aizel.cosmos_cli().evm_denom()
...
def test_ibc_transfer_from_eoa_with_internal_transfer(
    ...
    evm_denom = cli.evm_denom()
    gas_limit = 200_000
    aizel_gas_price = w3.eth.gas_price

    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    # address that will escrow the aaizel tokens when transferring via IBC
    ...
    eth_contract = setup_interchain_sender_contract(
        aizel, ACCOUNTS["signer2"], amt, initial_contract_balance
    )
    ...
    receipt = send_transaction(ibc.chains["aizel"].w3, send_tx, KEYS["signer2"])
    assert receipt.status == 1, debug_trace_tx(aizel, receipt.transactionHash.hex())
    ...
    if other_addr == src_addr:
        ...
        other_addr_final_balances = get_balances(ibc.chains["aizel"], other_addr_bech32)
        assert (
    ...
    src_final_balances = get_balances(ibc.chains["aizel"], src_bech32)
    ...
    contract_final_balance = get_balance(
        ibc.chains["aizel"], contract_bech32, evm_denom
    )
    assert (
    ...
    escrow_final_balances = get_balances(ibc.chains["aizel"], escrow_bech32)
...
def test_ibc_multi_transfer_from_eoa_with_internal_transfer(
    ...
    initial_contract_balance = int(1e18 / scaling_factor)
    eth_contract = setup_interchain_sender_contract(
        aizel, ACCOUNTS["signer2"], ibc_transfer_amt, initial_contract_balance
    )

    # send some funds (1e18 aaizel) to the contract to perform
    ...
    if err_contains is not None:
        ...
        src_final_amount_aizel = get_balance(ibc.chains["aizel"], src_bech32, evm_denom)

        exp_src_final_bal = amount_of(src_starting_balances, evm_denom) - fees
        assert src_final_amount_aizel == exp_src_final_bal, f"Failed {name}"
...
def test_multi_ibc_transfers_with_revert(ibc):
    ...
    src_denom = "aaizel"
    evm_denom = aizel.cosmos_cli().evm_denom()
    scaling_factor = get_scaling_factor(aizel.cosmos_cli())
    gas_limit = 800_000
    ...
    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    escrow_bech32 = ibc.chains["aizel"].cosmos_cli().escrow_address("channel-0")

    # Deployment of contracts and initial checks
    initial_contract_balance = int(1e18 / scaling_factor)
    interchain_sender_contract = setup_interchain_sender_contract(
        aizel, ACCOUNTS["signer2"], ibc_transfer_amt * 2, initial_contract_balance
    )
    ...
    # Check contracts final state (balance & counter)
    contract_final_balance = get_balance(
        ibc.chains["aizel"], interchain_sender_contract_bech32, evm_denom
    )
    assert (
        contract_final_balance
        == initial_contract_balance - exp_amt_tranferred_internally
...
def setup_interchain_sender_contract(
    aizel, src_acc, transfer_amt, initial_contract_balance
):
    """
    ...
    cli = aizel.cosmos_cli()
    evm_denom = cli.evm_denom()
    tranfer_denom = "aaizel"
    gas_limit = 200_000
    aizel_gas_price = aizel.w3.eth.gas_price
    # Deployment of contracts and initial checks
    eth_contract, tx_receipt = deploy_contract(aizel.w3, CONTRACTS["InterchainSender"])
    assert tx_receipt.status == 1

    counter = eth_contract.functions.counter().call()
    assert counter == 0

    pc = get_precompile_contract(aizel.w3, "ICS20I")
    # Approve the contract to spend the src_denom
    approve_tx = pc.functions.approve(
    ...
    tx_receipt = send_transaction(aizel.w3, approve_tx, src_acc.key)
    assert tx_receipt.status == 1
    ...
    fund_tx = cli.transfer(
        ...
        gas_prices=f"{aizel_gas_price + 100000/scaling_factor:.8f}{evm_denom}",
        generate_only=True,
    )

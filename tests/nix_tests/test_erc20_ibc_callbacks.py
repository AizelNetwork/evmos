...
def test_ibc_callbacks(
    ...
    bech32_aizel_addr = aizel_cli.address("signer2")
    ...
    with tempfile.NamedTemporaryFile("w") as fp:
        ...
        fp.flush()
        rsp = aizel_cli.gov_proposal("signer2", fp.name)
        assert rsp["code"] == 0, rsp["raw_log"]
        txhash = rsp["txhash"]

        wait_for_new_blocks(aizel_cli, 2)
        receipt = aizel_cli.tx_search_rpc(f"tx.hash='{txhash}'")[0]
        assert receipt["tx_result"]["code"] == 0, rsp["raw_log"]

    res = aizel_cli.query_proposals()
    props = res["proposals"]
    ...
    erc20_balance = contract.functions.balanceOf(aizel_addr).call()
    assert erc20_balance == initial_amt

    # convert to IBC voucher
    ibc_voucher_denom = f"erc20/{contract.address}"
    if convert_amt > 0:
        rsp = aizel_cli.convert_erc20(contract.address, convert_amt, "signer2")
        assert rsp["code"] == 0, rsp["raw_log"]
        wait_for_new_blocks(aizel_cli, 2)
    ...
    ibc_voucher_balance = get_balance(aizel, bech32_aizel_addr, ibc_voucher_denom)
    assert ibc_voucher_balance == convert_amt

    fee_denom = aizel_cli.evm_denom()
    scaling_factor = get_scaling_factor(aizel_cli)

    # send erc20 via IBC
    rsp = aizel_cli.ibc_transfer(
        bech32_aizel_addr,
        dst_addr,

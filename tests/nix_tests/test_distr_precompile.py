...
def test_fund_community_pool(aizel_cluster, name, deposit_amt, args, err_contains):
    """
    Test the fundCommunityPool function of the distribution
    precompile calling it from another precompile
    """
    gas_limit = 200_000
    gas_price = aizel_cluster.w3.eth.gas_price
    comm_pool_denom = "aaizel"
    ...
    eth_contract, tx_receipt = deploy_contract(
        aizel_cluster.w3, CONTRACTS["DistributionCaller"]
    )
    ...
    if deposit_amt is not None:
        ...
        # In case evm_denom != 'aaizel', we'll need
        # to fund the contract via a cosmos tx,
        # because the precompile only funds the community pool with 'aaizel'
        if fee_denom == comm_pool_denom:
            ...
            deposit_receipt = send_transaction(
                aizel_cluster.w3, deposit_tx, KEYS["signer1"]
            )
            assert deposit_receipt.status == 1, f"Failed: {name}"

            def check_contract_balance():
                new_contract_balance = aizel_cluster.w3.eth.get_balance(
            ...
            assert receipt["tx_result"]["code"] == 0, debug_trace_tx(
                aizel_cluster, txhash
            )

    signer1_prev_balances = get_balances(
        aizel_cluster, aizel_cluster.cosmos_cli().address("signer1")
    )
    signer2_prev_balances = get_balances(
        aizel_cluster, aizel_cluster.cosmos_cli().address("signer2")
    )

    # comm pool can have other coins balance (e.g. when evm_denom != 'aaizel')
    # For the precompiles, we only care about the 'aaizel' balance
    # because is the denomination used on the precompile tx
    pool_balances = aizel_cluster.cosmos_cli().distribution_community()
    ...
    receipt = send_transaction(aizel_cluster.w3, send_tx, KEYS["signer1"])

    if err_contains is not None:
        assert receipt.status == 0
        trace = debug_trace_tx(aizel_cluster, receipt.transactionHash.hex())
    ...
    assert receipt.status == 1, debug_trace_tx(
        aizel_cluster, receipt.transactionHash.hex()
    )

    # check that contract's balance is 0
    final_contract_balance = aizel_cluster.w3.eth.get_balance(eth_contract.address)
    ...
    # instead of 18
    fees = get_fee(aizel_cluster.cosmos_cli(), gas_price, gas_limit, receipt.gasUsed)

    final_pool_balances = aizel_cluster.cosmos_cli().distribution_community()

...
def custom_aizel(tmp_path_factory):
    ...
    yield from setup_aizel(path, 26800, long_timeout_commit=True)
...
def custom_aizel_6dec(tmp_path_factory):
    ...
    # run with long timeout commit to ensure all
    # txs are included in the same block
    yield from setup_aizel_6dec(path, 46880, long_timeout_commit=True)
...
def custom_aizel_rocksdb(tmp_path_factory):
    ...
    # run with long timeout commit to ensure all
    # txs are included in the same block
    yield from setup_aizel_rocksdb(path, 26810, long_timeout_commit=True)
...
def test_priority(aizel_cluster):
    """
...
def test_native_tx_priority(aizel_cluster):

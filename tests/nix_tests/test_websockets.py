def test_single_request_netversion(aizel_cluster):
    aizel_cluster.use_websocket()
    eth_ws = aizel_cluster.w3.provider
...
def test_batch_request_netversion(aizel):
    return


def test_ws_subscribe_log(aizel):
    return


def test_ws_subscribe_newheads(aizel):
    return

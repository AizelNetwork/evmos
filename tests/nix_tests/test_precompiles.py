...
def test_ibc_transfer(ibc):
    ...
    old_src_balances = get_balances(ibc.chains["aizel"], src_addr)
    ...
    gas_estimation = ibc.chains["aizel"].w3.eth.estimate_gas(tx)
    receipt = send_transaction(ibc.chains["aizel"].w3, tx, KEYS["signer2"])
    ...
    fee = get_fee(cli, aizel_gas_price, gas_estimation, receipt.gasUsed)
...
def test_ibc_transfer_invalid_packet(ibc):
    ...
    old_src_balance = get_balance(ibc.chains["aizel"], src_addr, src_denom)
    ...
    try:
        tx = pc.functions.transfer(
            ...
            0,
            "",
        ).build_transaction({"from": ADDRS["signer2"], "gasPrice": aizel_gas_price})
        send_transaction(ibc.chains["aizel"].w3, tx, KEYS["signer2"])
    except Exception as error:
        assert error.args[0]["message"] == f"rpc error: code = Unknown desc = {exp_err}"

        new_src_balance = get_balance(ibc.chains["aizel"], src_addr, src_denom)
...
def test_staking(ibc):
    ...
    old_src_balances = get_balances(aizel, del_addr)
    ...
    tx = pc.functions.delegate(ADDRS["signer2"], validator_addr, amt).build_transaction(
        {"from": ADDRS["signer2"], "gasPrice": aizel_gas_price}
    )
    gas_estimation = aizel.w3.eth.estimate_gas(tx)
    ...
    new_src_balances = get_balances(aizel, del_addr)
...
def test_staking_via_sc(ibc):
    ...
    approve_tx = contract.functions.testApprove(
        ...
        {"from": ADDRS["signer1"], "gasPrice": aizel_gas_price, "gas": gas_wanted}
    )

    gas_estimation = aizel.w3.eth.estimate_gas(approve_tx)
    ...
    fee1 = get_fee(cli, aizel_gas_price, gas_wanted, receipt.gasUsed)
    ...
    gas_estimation = aizel.w3.eth.estimate_gas(delegate_tx)
    ...
    fee2 = get_fee(cli, aizel_gas_price, gas_wanted, receipt.gasUsed)

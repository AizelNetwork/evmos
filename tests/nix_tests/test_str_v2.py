...
from .network import CosmosChain, Aizel
from .utils import (
    ADDRS,
    KEYS,
    WEVMOS_ADDRESS,
...
# uatom from cosmoshub-2 -> cosmoshub-1 IBC representation on the Aizel chain.
...
# The ERC20 address of ATOM on Aizel
...
UPDATE_PARAMS_PROP = {
    "messages": [
        {
            "@type": "/aizel.erc20.v1.MsgUpdateParams",
...
def ibc(request, tmp_path_factory):
    ...
    network = prepare_network(path, name, [aizel_build, "cosmoshub-1", "cosmoshub-2"])
    yield from network
...
def test_str_v2_single_hop(ibc):
    ...
    w3 = aizel.w3
    aizel_cli = aizel.cosmos_cli()
    aizel_addr = ADDRS["signer2"]
    bech_dst = eth_to_bech32(aizel_addr)

    gaia_cli = gaia.cosmos_cli()
    gaia_addr = gaia_cli.address("signer2")

    # Before IBC transfer, check no dynamic precompiles available
    active_dynamic_precompiles = aizel_cli.erc20_params()["params"][
        "dynamic_precompiles"
    ]
    ...
    old_dst_balance = get_balance(aizel, bech_dst, ATOM_IBC_DENOM)
    rsp = gaia_cli.ibc_transfer(
        gaia_addr, bech_dst, "5000uatom", "channel-0", 1, fees="10000uatom"
    )
    assert rsp["code"] == 0

    wait_for_ack(aizel_cli, "Aizel")

    pairs = aizel_cli.get_token_pairs()
    active_dynamic_precompiles = aizel_cli.erc20_params()["params"][
        "dynamic_precompiles"
    ]
    new_dest_balance = get_balance(aizel, bech_dst, ATOM_IBC_DENOM)
    erc_dest_balance = erc20_balance(w3, ATOM_1_ERC20_ADDRESS, aizel_addr)
...
def test_str_v2_multi_hop(ibc):
    ...
    aizel_old_balance = get_balance(aizel, bech_dst, ATOM_2_IBC_DENOM_MULTI_HOP)
    ...
    aizel_balance = get_balance(aizel, bech_dst, ATOM_2_IBC_DENOM_MULTI_HOP)
    dynamic_precompiles = aizel_cli.erc20_params()["params"]["dynamic_precompiles"]
    token_pairs = aizel_cli.get_token_pairs()

    # Here it's only one from the previous one we've registered in the first test
    assert aizel_old_balance + 50000 == aizel_balance
...
def test_waizel_precompile_transfer(ibc):
    ...
    assert aizel_balance_after == aizel_balance + 1000000
...
def test_toggle_erc20_precompile(ibc):
    ...
    acc = aizel_cli.evm_account(contract_bech32_addr)
...
def update_erc20_params(aizel: Aizel, native_precomiles=[], dynamic_precompiles=[]):
    ...
    approve_proposal(aizel, props[props_count - 1]["id"])
    wait_for_new_blocks(cli, 2)

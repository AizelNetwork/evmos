...
var (
	...
	toAddr           = aizelutiltx.GenerateAddress()
	funderAddr       = aizelutiltx.GenerateAddress()
	diffFunderAddr   = aizelutiltx.GenerateAddress()
...
func (s *PrecompileTestSuite) TestCreateClawbackVestingAccount() {
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() []interface{} {
				differentAddr := aizelutiltx.GenerateAddress()
				return []interface{}{
					funderAddr,
					differentAddr,
					false,
	...
	testCases := []struct {
		...
		},
		{
			...
			func() []interface{} {
				...
				err = aizelutil.FundAccount(ctx, s.network.App.BankKeeper, toAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(baseDenom, math.NewInt(100))))
				return []interface{}{
					s.keyring.GetAddr(0),
					toAddr,
					uint64(time.Now().Unix()), //nolint:gosec // G115

...
type IntegrationTestSuite struct {
	...
	aizelAddr, xmplAddr   common.Address
...
func (is *IntegrationTestSuite) SetupTest() {
	...
	is.aizelAddr = common.HexToAddress(tokenPair.Erc20Address)
...
var _ = Describe("Bank Extension -", func() {
	var (
		...
		aizelTotalSupply, _ = new(big.Int).SetString("200003000000000000000000", 10)
	...
	Context("Direct precompile queries", func() {
		...
		Context("totalSupply query", func() {
			It("should return the correct total supply", func() {
				...
				Expect(balances[0].Amount).To(Equal(aizelTotalSupply))
				Expect(balances[1].Amount).To(Equal(xmplTotalSupply))
			})
		})

		Context("supplyOf query", func() {
			It("should return the supply of Aizel", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(directCall, contractData, bank.SupplyOfMethod, is.aizelAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")
				...
				Expect(out[0].(*big.Int)).To(Equal(aizelTotalSupply))
			})
	...
	Context("Calls from a contract", func() {
		...
		Context("supplyOf query", func() {
			It("should return the supply of Aizel", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(contractCall, contractData, SupplyOfFunction, is.aizelAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")

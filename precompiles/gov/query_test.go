...
var (
	...
	TestProposalMsgs = []sdk.Msg{
		banktypes.NewMsgSend(govAcct, addr, sdk.NewCoins(sdk.NewCoin(aizeltypes.BaseDenom, math.NewInt(1000)))),
	}
)
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			if tc.expPass {
				...
				s.Require().Equal([]cmn.Coin{{Denom: "aaizel", Amount: big.NewInt(100)}}, out.Deposit.Amount)
			} else {
				s.Require().Error(err)
				s.Require().Contains(err.Error(), tc.errContains)
			}
